...
En el siguiente informe se presentan una serie de pruebas con distintos algoritmos de machine learning con un conjunto de datos con información sobre las características de ditintas viviendas, esta información será utilizada para entrenar a los modelos de machine learning. El objetivo de este informe es hallar un algoritmo de machine learning que sea de ayuda para la empresa a dar mejoresvaloraciones sobre las viviendas. 

### Análisis exploratorio

Prmero se realizó una exploración inicial del conjunto de datos, el primer paso fue analizar cada una de las variables
...
Luego fue necesaria una visualización inicial de la información 

```{r}
summary(train[,-1])
```

Del resumen se obtuvieron estos detalles interesantes:

- En promedio, se han vendido más casas que fueron construidas en el año 1971. La casa más moderna que se ha vendido en el datset fue construida en el año 2010. 
...
Luego de encontrar los detalles más generales del conjunto de datos, fue necesaria una limpieza pues varias columnas contenían valores NA tanto en variables numéricas como en variables categóricas. 
```{r}
library(dplyr)
...
Algunas variables como PoolQC, Fence, MiscFeature o FireplaceQu mostraron una gran cantidad de valores NA's, por lo que fueron eliminados. Otras variables como GarageQual, se tomará el NA como inexistente. 



Luego las variables ordinales fueron transformadas e variables numéricas. Algunas variables nominales como el vecindario tabién fueron trasnforadas ya que estas pueden jugar un papel clave en la valoración de una vivienda. 
...
La variable respuesta es el precio de las viviendas (SalePrice en el dataset) ya que el precio de la vivienda es el factor que determina la valoración de la casa . Es necesario determinar si la variable respuesta contiene una distribución normal ya que de ser cierto, podría facilitar la interpretación de la información y la media y la desviación estándar podrián ser utilizadas de una manera más confiable. 

Para determinar la distribución de la variable respuesta, se utilizarán varios métodos como un histograma, pruebas de normalidad y un qqplot. 
...
```
Se obtiene un p-value de 2.2e-16, este es un valor extremadamente bajo, lo cuál confirma que la variable respuesta no tiene una distribución normal,  por lo que se usará la función log para transformar los datos. 
...
Es importante determinar cuáles son las variables que afectan de manera más significativa a la variable respuesta ya que luego el uso de estas variables facilitará la modelación de los algoritmos de machine learning. 
...
Sabiendo cuáles son las variales que afectan de manera significativa a la variable respuesta, fue necesario realizar un modelo de regresión lineal inicial para predecir los precios de las viviendas. 
...
Luego de generar el modelo, se hizo un análisis de los resultados. 
...
**En la primera gráfica** (Residuals vs Fitted), se observa que los residuos no están completamente dispersos de manera aleatoria alrededor de la línea horizontal en cero. Hay una ligera curvatura, lo que sugiere una posible no linealidad en los datos. Esto indica que el modelo puede no capturar completamente la relación entre las variables predictoras y el precio de venta.
...
Luego, fue necesaria una división del conjunto de entrenamiento para obtener datos que fueran de ayuda para realizar pruebas de los modelos generados. 
```{r}
set.seed(3915)
trainIndex <- createDataPartition(train$SalePrice, p = 0.8, list = FALSE)
train_set <- train[trainIndex, ]
...
También fue necesaria una limpieza adicional eliminando las variables categóricas que solo tienen un nivel o factor. 
```{r}
...
Se genraron 2 modelos de regresión, uno con todas las variables influyentes y otro con solo la variable más influyente en la variable respuesta. 
```{r}
...
Un análisis de grupos fue necesario para determinar si hay datos que comparten información similar entre sí y pueden ser clasificados en una misma categoría. Para esto se utilizaron las funciones de la librería fbiz para hacer clustering con el algoritmo k-means. 
...
El k-optimo elegido fue k=3 debido a que tanto en el método del codo, como en el de silueta lo favorecen.
```{r crear_clusters}
k_optimo <- 3
...
- Las viviendas de precio medio-alto (Cluster 3): precios medios y mayor variabilidad en la información. 



### Análisis de algoritmos

Ahora, se mostraran los resultados obtenidos con distintos algoritmos de machine learning

#### Árboles decisión y random forest

Los primeros algoritmos a analizar serán el random forest y los árboles de decisión, los árboles de decisión generan nodos que son capaces de dividir y clasificar la información en distintas categorías mientras que el random forest genera una serie de árboles de decisión y en base a los resultados de estos árboles, genera una respuesta. 
